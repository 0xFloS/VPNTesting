...
def fetch_server_list():
    ...
    command = r'''curl -s "https://api.nordvpn.com/v2/servers?limit=0" | jq -r '.[] | select(.hostname | startswith("us")) | {id, name, station, hostname, status} | "\(.id)\t\(.name)\t\(.station)\t\(.hostname)\t\(.status)"' '''
